...
/**
 * Tests for {@link ArticleExtractorService}
 *
 * @author watsond
 */
public class ArticleExtractorServiceTest {
  ...
  private DefaultingLanguageDetector languageDetector;
  ...
  public void before() {
    ...
    languageDetector = mock(DefaultingLanguageDetector.class);
  ...
  public void testArticleExtract() {
    ...
    article
        .setMetaDescription("South Sudan's government says it is sending more troops and police to the town ...");
    article
        .setCleanedArticleText("South Sudan's government says it is sending more troops and police to the town ...");

    when(networkHelper.getHtml(url)).thenReturn("article html");
    when(goose.extractContent(Matchers.eq(url), Matchers.anyString(), Matchers.anyString()))
        .thenReturn(article);

    net.matthaynes.juicer.service.ArticleExtractorService.Article extracted = articleExtractorService
        .extract(url);
